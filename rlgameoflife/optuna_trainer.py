...
class OptunaAgentTrainer:
    def __init__(self):
        ...
        self.agent_trainer = agent.AgentTrainer()
    ...
    def objective(self, trial : optuna.Trial):
        agent_parameters = agent.AgentTrainerParameters(
            ...
            num_episodes=trial.suggest_int("num_episodes", 40, 100, log=True),
            max_steps_per_episode=trial.suggest_int("max_steps_per_episode", 200, 600, log=True),
            replay_memory_size=trial.suggest_int("replay_memory_size", 1000, 10000),
        )
        agent_trainer = agent.AgentTrainer(agent_parameters)
        return agent_trainer.train()
    ...
    def optimize(self):
        ...
        complete_trials = self.study.get_trials(deepcopy=False, states=[optuna.trial.TrialState.COMPLETE])
